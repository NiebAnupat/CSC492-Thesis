...
export class AuthController {
  ...
  async developer_register(@Body() createDeveloperDto: CreateDeveloperDto, @Res() res: Response) {
    const { access_token } = await this.authService.developer_register(createDeveloperDto);
    this.setAccessTokenCookie(res, access_token);
    res.status(HttpStatus.CREATED).send(
      {
